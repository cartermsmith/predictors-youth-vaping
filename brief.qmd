...
This project uses data from the [Canadian Community Health Survey: Public Use Microdata File](https://doi.org/10.25318/82m0013x-eng). The data are from the 2022 report, and are based on reports from over 130,000 people across Canada aged 12+. This project includes data from 447 youth respondents. 
...
The youth sample was queried from the full microdata file using SQL, via the `duckdb` library. Subsequent cleaning, modelling, and visualization was done in Python. The complete analysis code can be viewed as an HTML file [here](github.com).
...
Only two predictors meaningfully altered the likelihood of e-cigarette use. One explanation for this was that the predictors were highly correlated, violating a key assumption of logistic regression. After examining the variance inflation factor of each predictor, many of the variables were closely correlated with each other. A random forest classifier, which does not make this assumption, allowed me to examine predictor importance.

However, upon checking the model's diagnostics, I found that this model did not learn much from the data. The model was only doing slightly better than random guessing when making predictions (AUC = 0.52). This means that the non-significant predictors likely lack predictive power on the outcome. The model's predictor rankings should be interpreted cautiously.

I concluded that the null effects in the logistic model were driven by few signals in a noisy dataset, rather than violation of a statistical assumption.

The significant predictors in the logistic model remain as strong signals that may guide programs aiming to reduce youth vaping by targeting certain measures of well-being like community belonging or life satisfaction.